...
class ProductUploadWizard {
    ...
    showScreen(screenId) {
        ...
        screens.forEach(id => {
            ...
            if (el) {
                if (id === screenId) {
                    el.style.display = (id === 'screen-loading' || id === 'screen-success') ? 'flex' : 'block';
                } else {
    ...
    handleFinalSubmission(event) {
        ...
        .then(data => {
            if (data.success) {
                // Show success screen and trigger fade in
                this.showScreen('screen-success');
                const successScreen = document.getElementById('screen-success');
                successScreen.style.display = 'flex'; // override default block
                ...
                // Wait 2 seconds, fade out, then redirect
                setTimeout(() => {
                    successScreen.style.opacity = '0';
                    setTimeout(() => {
                        const redirectUrl = data.redirect_url || successScreen.getAttribute('data-redirect-url') || '/catalogue/products/';
                        window.location.replace(redirectUrl);
                    }, 500); // Wait 0.5s for fade out animation
                }, 2500); // 2 seconds + initial 0.5s fade in time
            } else {
